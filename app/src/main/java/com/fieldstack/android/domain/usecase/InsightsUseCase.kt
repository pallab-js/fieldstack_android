...
class InsightsUseCase @Inject constructor(
    ...
    private val weekAgo = Instant.now().minus(7, ChronoUnit.DAYS)

    operator fun invoke(userId: String): Flow<WeeklyInsights> =
        repository.observeTasks(userId).map { tasks ->
            ...
            WeeklyInsights(
                ...
                reportsSubmitted = tasks.count { it.status == TaskStatus.Completed }, // proxy
